<template>
  <div>
    <p class="showUnix" v-cloak>
      当前的Unix时间戳是：<span>{{ timeStamp }} </span>当前时间：<span>{{
        currentTime
      }}</span>
    </p>
    <div class="unix2date">
    ...
    <div class="date2unix">
      时间（年/月/日 时:分:秒）：
      <el-date-picker
        v-model="date2unixEnter"
        :value="date2unixEnter"
        type="datetime"
        placeholder="选择日期时间"
...
export default {
  ...
  methods: {
    convertDateTime() {
      if (this.selectVal == "秒") {
        this.unix2dateResult = dayjs
          .unix(parseInt(this.unix2dateEnter))
          .format("YYYY年MM月DD日 HH:mm:ss");
      } else {
        this.unix2dateResult = dayjs(parseInt(this.unix2dateEnter)).format(
