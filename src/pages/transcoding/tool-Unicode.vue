...
export default {
  ...
  methods: {
    ...
    Native2Unicode() {
      ...
      this.bottomInput = "";
      for (var i = 0; i < this.topInput.length; i++)
        this.bottomInput += "&#" + this.topInput.charCodeAt(i) + ";";
    },
    //Unicode 转换 Native
    Unicode2Native() {
      ...
      for (var i = 0; i < code.length; i++)
        this.topInput += String.fromCharCode(code[i].replace(/[&#;]/g, ""));
    },
    //Native 转换 UTF-8
    Native2UTF8() {
      if ("" == this.topInput) {
      ...
      this.bottomInput = this.topInput.replace(
        /[^\u0000-\u00FF]/g,
        function ($0) {
    ...
    //UTF-8 转换 Native
    UTF82Native() {
      this.topInput = unescape(
        this.bottomInput.replace(/&#x/g, "%u").replace(/;/g, "")
      );
    },
    //Native 转 ASCII
    native2ascii() {
